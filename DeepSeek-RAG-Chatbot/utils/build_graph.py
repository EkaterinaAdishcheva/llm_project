...
def build_knowledge_graph(docs):
    G = nx.Graph()
    for doc in docs:
        entities = re.findall(r'\b[А-Я][а-я]+(?: [А-Я][а-я]+)*\b', doc.page_content)
...
#    st.write(f"🔎 Searching GraphRAG for: {query}")
    print(f"🔎 Searching GraphRAG for: {query}")
    ...
    if matched_nodes:
        ...
        print(f"🟢 GraphRAG Matched Nodes: {matched_nodes}")
        print(f"🟢 GraphRAG Retrieved Related Nodes: {related_nodes[:top_k]}")
        return related_nodes[:top_k]
    
    # st.write(f"❌ No graph results found for: {query}")
    print(f"❌ No graph results found for: {query}")
    return []